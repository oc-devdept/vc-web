...
const MegaTab = () => {
  ...
  const _RenderModel = () => {
    ...
    return AllModels[1].map((model, index) => {
      return (
        ...
        >
          <div className="col-6 col-sm-4 col-md-3 col-lg-2 my-2 px-2">
  ...
  return (
    <div className="text-uppercase">
      ...
      {!Loading && (
        <>
          <ul className="nav nav-tabs">{_RenderAllItems()}</ul>

          <div className="tab-content">
            {MegaMenu.length > 0 && <div className="row">{_RenderModel()}</div>}
          </div>
        </>
      )}
    </div>
