...
const LoanCalculator = props => {
  ...
  const handleCalculationClick = () => {
    const totalInterest = (parseFloat(loanAmount) * (parseFloat(interestRate) / 100 + 1) - parseFloat(loanAmount)) * (loanTerm / 12)
    const monthlyInstallment = (totalInterest + parseFloat(loanAmount)) / loanTerm
    ...
    <div className="calculator p-3">
      ...
      <div className='calculator-submit row justify-content-center'>
        {!!loanAmount && !!interestRate
          ? 
          <Button onClick={handleCalculationClick}>
            Calculate
          </Button>
          :
